...
# Grounding DINO
import sys;sys.path.append("/mnt/ve_share/liushuai/panoptic-segment-anything")
...
from pinmask import maskpinner,hoimaskpinner
...
def generate_panoptic_mask(
    ...
    pinner=hoimaskpinner()
    import cv2
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    import tqdm
    for dirpath, dirnames, filenames in os.walk("/mnt/ve_share/liushuai/panoptic-segment-anything/hoi4d/HOI4D_annotations"):
        if 'mask' in dirpath:
            a=dirpath.split('/')
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            videoname=a[-8]+a[-7]+a[-6]+a[-5]+a[-4]+a[-3] #video的命名
            out = cv2.VideoWriter(f'/mnt/ve_share/liushuai/panoptic-segment-anything/hoi4d/{videoname}.mp4', fourcc, 5, (1920,1080))
            instance=a[-7]
            # resultdict=pinner.getindex(pinner.category2label_map_instanceseg[instance],pinner.category2label_map[instance])
            # num_palette=len(resultdict.keys())
            # sub_palette = random.sample(range(1, 256), num_palette)
            # for i in range(len(list(resultdict.keys()))):
            #     resultdict[list(resultdict.keys())[i]].append(pinner.palette[sub_palette[i]])
            videopath='/mnt/ve_share/liushuai/panoptic-segment-anything/hoi4d/HOI4D_release'
            for ele in range(7,len(dirpath.split("/"))-2):
                videopath=os.path.join(videopath,dirpath.split("/")[ele])
            videopath=os.path.join(videopath,"align_rgb","image.mp4")
            cap = cv2.VideoCapture(videopath)
        # 定义要拆分的帧数
            num_frames = 300
            interval = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) / num_frames)
            tick=0 #计数，记录视频的帧数
            for ele in tqdm.tqdm(sorted(filenames)):
                cap.set(cv2.CAP_PROP_POS_FRAMES, tick*interval)
                _,frame = cap.read() #读视频
                tick+=1
                if tick %3 !=0:
                    continue
                targetdir=os.path.join(dirpath,ele)
        ...
        # detect boxes for "thing" categories using Grounding DINO
            ...
            panoptic_inds, thing_category_ids, stuff_category_names, category_name_to_id
                ...
                pinnedmask=np.zeros_like(panopticmask)
                colordict={}
                result=pinner.get_mask_and_label(targetdir)
                for num in range(len(result[2])):
                    mask,cate,id=result[0][num],result[1][num],result[2][num]
                    pinnedmask+=mask*id
                    colordict[id]=[[0],pinner.categories[str(cate)]]
                pinnedmask=pinnedmask.astype('uint8')
                panopticmask[pinnedmask!=0]=pinnedmask[pinnedmask!=0]
                category=''
                for uniqueele in np.unique(panopticmask):
                    if uniqueele not in colordict.keys():
                        for anno in annotations:
                            if anno['id']==uniqueele:
                                category=category_names[int(anno['category_id'])]
                                break
                        colordict[uniqueele]=[[0],category]
                i=1
                for key in colordict.keys():
                    colordict[key][0]=pinner.palette[key%256]
                    i+=1
                finalimage=pinner.drawmask(frame,panopticmask,colordict,ele)
...
if __name__ == "__main__":
    things="table, toy, outlet"
    stuff="floor, ground, wall, window, curtain"
    generate_panoptic_mask(things,stuff)
